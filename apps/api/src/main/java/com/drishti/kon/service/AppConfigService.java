...
public class AppConfigService {
    ...
    public LinksResponse getLinks() {
        return new LinksResponse(
                getValue(KEY_BUG_REPORT_URL),
                getValue(KEY_FEATURE_REQUEST_URL),
                getValue(KEY_REPORT_POST_URL)
        );
    }

    public LinksResponse updateLinks(UpdateLinksRequest request, User requester) {
        requireModerator(requester);
        if (request.getBugReportUrl() != null)      setValue(KEY_BUG_REPORT_URL, request.getBugReportUrl());
        if (request.getFeatureRequestUrl() != null) setValue(KEY_FEATURE_REQUEST_URL, request.getFeatureRequestUrl());
        if (request.getReportPostUrl() != null)     setValue(KEY_REPORT_POST_URL, request.getReportPostUrl());
        return getLinks();
    }

    private String getValue(String key) {
        return appConfigRepository.findByKey(key)
                .map(AppConfigItem::getConfigValue)
                .orElse(null);
    }

    private void setValue(String key, String value) {
        AppConfigItem item = appConfigRepository.findByKey(key)
                .orElseGet(() -> {
                    AppConfigItem c = new AppConfigItem();
                    c.setConfigKey(key);
                    return c;
                });
