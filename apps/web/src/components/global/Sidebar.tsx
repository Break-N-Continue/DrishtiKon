    ...
    icon: (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M7 8h10M7 12h6M7 16h4m6-8l2 2m0 0l-2 2m2-2H3"
        />
      </svg>
    ),
  },
...
export default function Sidebar({
  ...
  return (
    <div
      className={`transition-all duration-300 bg-surface flex flex-col h-full w-full`}
    >
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Toggle button at top */}
        <div
          className={`flex items-center ${collapsed ? "justify-center py-4 px-2" : "justify-between py-4 px-4"}`}
        >
          {!collapsed && (
            <p className="text-on-surface-variant font-semibold text-xs tracking-wider">
              MENU
            </p>
          )}
          <button
        ...
        <nav className="flex-1 overflow-y-auto py-2 flex flex-col space-y-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={collapsed ? "scale-110" : ""}>
                  {item.icon}
                </span>
        ...
        <div
          className={`border-t border-outline-variant bg-surface-container-low ${collapsed ? "p-2" : "p-3"}`}
        >
          {user ? (
            <div
              className={`flex items-center gap-3 ${collapsed ? "flex-col justify-center" : "justify-start"}`}
            >
